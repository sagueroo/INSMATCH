...
// Now wrap renderMatchsPage logic inside a drawer popup component.
// We will replace the entire renderMatchsPage function body.
const oldMatchPageRegex = new RegExp("const renderMatchsPage = \\\\(\\\\) => \\\\([\\\\s\\\\S]*?\\\\n  // ━━━━━━━━━━━━ POPUP DETAIL ━━━━━━━━━━━━", "m");
const matchMatch = content.match(oldMatchPageRegex);
if (matchMatch) {
  let innerMatchPage = matchMatch[0];
  // extract content between "<div style={{ flex: 1, overflowY: 'auto'" and the final "    </div>"
  // Actually, we can just replace 'renderMatchsPage = () => (' with 'renderMatchsDrawer = () => { if (!isMatchDrawerOpen) return null; return ('
  innerMatchPage = innerMatchPage.replace(
    /const renderMatchsPage = \(\) => \(\s*<div style={{ display: 'flex', flexDirection: 'column', height: '100%', overflow: 'hidden' }}>/g,
  ...
  innerMatchPage = innerMatchPage.replace(
    /<span style={{.*?}}>\{requests\.length\}<\/span>\n\s*<\/div>\n\s*<\/div>/,
  ...
  // Close the drawer tags at the end instead of just one </div>
  innerMatchPage = innerMatchPage.replace(
    /\s*<\/div>\n\s*<\/div>\n\s*\);\n\n  \/\/ ━━━━━━━━━━━━ POPUP DETAIL ━━━━━━━━━━━━/,
    "\n        </div>\n      </div>\n      </div>\n    );\n  };\n\n  // ━━━━━━━━━━━━ POPUP DETAIL ━━━━━━━━━━━━"
  );
  
  content = content.replace(oldMatchPageRegex, innerMatchPage);
}
